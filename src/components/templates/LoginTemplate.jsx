...
export function LoginTemplate() {
  ...
  //se carga el tema light por defecto para que se detecte las letras al introducir el email y la contraseña

  useEffect(() => {
    setTheme("light");
  }, [setTheme]);

  const { signInWithEmail } = useAuthStore(); //iniciar sesion con email y contraseña
  const [state, setState] = useState(false);
  const [stateInicio, setStateInicio] = useState(false);
  const navigate = useNavigate();
  //se usa el hook useForm de react-hook-form para manejar los formularios
  const {
    register,
    formState: { errors },
    handleSubmit,
  } = useForm();
  //el data viene de los inputs del formulario que se utilizo el register de react-hook-form con el supabase
  //signInWithEmail es una funcion que se encarga de iniciar sesion con email y contraseña y verifica si los datos son correctos
  async function iniciar(data) {
    ...
    if (response) {
      ...
      //si la respuesta es false, significa que los datos son incorrectos
      setStateInicio(true);
    }
  }
          ...
          <form onSubmit={handleSubmit(iniciar)}>
            <InputText icono={<v.iconoemail />}>
              <input
                ...
                type="text"
                placeholder="email"
              //Utilizamos register de react-hook-form para manejar el formulario
                {...register("correo", {
                  required: true,
                })}
              />
